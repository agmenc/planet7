...
trait RowSupport {
  case class Row(values: Seq[(String, String)])  {
    ...
    def renameAndRestructure(columns: Seq[(String, String)]): Row = {
      val theRow = Map[String, String](values:_*)
      Row(columns map { case (before, after) =>
        if (theRow.contains(before)) after -> theRow(before)
        else after -> ""
      })
    }
  ...
  object RowTransforms {
    ...
    def renameAndRestructure(columnNames: Seq[(String,String)]): Row => Row = row => row.renameAndRestructure(columnNames)
  }
